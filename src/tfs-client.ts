...
export class TfsClient {
  ...
  /**
   * Lấy chi tiết nhiều work items theo IDs với relations (để biết child links).
   */
  async getWorkItemsByIdsWithRelations(ids: number[]): Promise<WorkItem[]> {
    if (ids.length === 0) return [];
    // Gọi từng item với $expand=relations vì batch endpoint có thể không hỗ trợ
    const results: WorkItem[] = [];
    for (const id of ids.slice(0, 50)) {
      try {
        const res = await this.http.get(`/_apis/wit/workitems/${id}`, {
          params: {
            '$expand': 'relations',
          },
        });
        results.push(res.data as WorkItem);
      } catch {
        // skip item nếu lỗi
      }
    }
